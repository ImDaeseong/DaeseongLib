...
func GoogleMapsPlaceSearch(url string) bool {
	...
	for _, val := range result.Results {
		sUrl := fmt.Sprintf("https://maps.googleapis.com/maps/api/place/details/json?placeid=%s&key=%s", val.PlaceID, API_Key)
		writeGoogleMapString("GooglePlacesInfo.txt", sUrl)
	}
	return true
}
