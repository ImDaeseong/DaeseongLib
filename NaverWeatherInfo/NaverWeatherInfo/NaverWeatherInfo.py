...
url = "http://weather.naver.com/rgn/cityWetrCity.nhn?cityRgnCd=CT"
...
def getVal(sInput, sFirst, sEnd):
    indexf = sInput.find(sFirst)
    indexl = sInput.find(sEnd)
    return sInput[indexf + len(sFirst):indexl]


def loadurl():
    global blind
    citynum = city['서울']
    url = "http://weather.naver.com/rgn/cityWetrCity.nhn?cityRgnCd=CT" + citynum
    responses = requests.get(url)
    soup = BeautifulSoup(responses.text, 'html.parser')

    # today_weather = soup.findAll('div', {'class': 'today_weather'})
    # print(today_weather)

    weather_area = soup.findAll('div', {'class': 'weather_area'})
    # print(weather_area)
    for item in weather_area:
        current = item.find('strong', {'class': 'current'})
        degree_height = item.find('strong', {'class': 'degree_height'})
        degree_low = item.find('strong', {'class': 'degree_low'})

    if current is not None:
        # print(current)
        val1 = between(str(current), '</span>', '</span>')
        val2 = getVal(val1, '</span>', '<span')
        print("현재온도:" + val2)

    if degree_height is not None:
        # print(degree_height)
        val1 = getVal(str(degree_height), '</span>', '</strong>')
        print("최고온도:" + val1)

    if degree_low is not None:
        # print(degree_low)
        val1 = getVal(str(degree_height), '</span>', '</strong>')
        print("최저온도:" + val1)
